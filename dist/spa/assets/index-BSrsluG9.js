...
function VideoPlayer() {
  // Référence à la balise <video> pour contrôler la lecture, le mute, etc.
  const videoRef = useRef<HTMLVideoElement>(null); 
  // État pour gérer l'icône du son (true = coupé)
  ...
  function toggleFullscreen() {
    ...
    if (videoElement) {
      ...
      } else if ((videoElement as any).webkitRequestFullscreen) { /* Chrome, Safari and Opera */
  ...
  return (
    <div
      className="relative w-full max-w-4xl mx-auto rounded-xl overflow-hidden ring-1 ring-border bg-black"
    >
      <video
        ref={videoRef}
        className="w-full h-full aspect-video object-cover"
        // ----------------------------------------------------------------------
        // NOUVELLE SOURCE DE VOTRE VIDÉO DANS dist/spa/
        src="/teaser EMC.mp4" 
        // ----------------------------------------------------------------------
        autoPlay={true}
        muted={true}
      ...
      {/* Bouton Mute/Unmute (restauré de l'ancienne fonction qk) */}
      <button
        onClick={toggleMute}
      ...
      {/* NOUVEAU: Bouton Plein Écran (en bas à droite) */}
      <button
